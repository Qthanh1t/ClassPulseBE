...
- Unique partial index: `UNIQUE (classroom_id) WHERE status = 'active'` trên `sessions` — ngăn 2 session active cùng lúc ở DB level (V11)
...
| T059 | SessionService (start) — **idempotent**: nếu đã có active session → trả về session đó kèm wsTicket mới (không throw error); nếu chưa có → tạo mới với `saveAndFlush`; race condition bắt bằng `DataIntegrityViolationException` từ unique partial index (V11) → query và trả session hiện có; Flyway V11: `UNIQUE (classroom_id) WHERE status = 'active'`; thêm `existsBySchedule_Id` vào SessionRepository + fix `ScheduleService.delete` M06 TODO | `session/SessionService.java`, `session/SessionRepository.java`, `schedule/ScheduleService.java`, `db/migration/V11__add_unique_active_session_per_classroom.sql` |
